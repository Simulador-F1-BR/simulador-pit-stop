...
def _force_reload_session(year: int, round_number: int, session_type: str = "R"):
    """Limpa o cache e recarrega com retry em caso de rate limit."""
    _load_session_cached.clear()

    max_attempts = 3
    for attempt in range(max_attempts):
        try:
            return _load_session_cached(year, round_number, session_type)
            ...
            if attempt < max_attempts - 1:
                ...
                st.error(
                    "Não foi possível carregar a sessão após várias tentativas. "
                    "O FastF1 está limitando as requisições. "
...
def _session_is_alive(session) -> bool:
    """Verifica se os dados da sessão estão realmente carregados."""
    try:
        laps = session._laps  # acessa o atributo interno diretamente
...
def load_session(year: int, round_number: int, session_type: str = "R"):
    ...
    if not _session_is_alive(session):
        session = _force_reload_session(year, round_number, session_type)
...
def get_driver_laps(session, driver_code, session_type: str = "R"):
    try:
        ...
        # Objeto morto chegou aqui mesmo assim — informa o usuário claramente
        st.error(
            "Os dados da sessão foram perdidos (o app ficou inativo por muito tempo). "
